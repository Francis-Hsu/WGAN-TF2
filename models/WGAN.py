...
class WGAN:
    ...
    def lipschitz_penalty(self, x, x_hat):
        ...
        with tf.GradientTape() as D_tape:
            ...
            y_tilde = self.D(x_tilde, training=True)
        d_grad = D_tape.gradient(y_tilde, x_tilde)
        grad_norm = tf.norm(tf.reshape(d_grad, shape=[x.shape[0], -1]), axis=1)
    ...
    def train_discriminator(self, x_batch):
        with tf.GradientTape() as D_tape:
            ...
            d_obj = tf.math.reduce_mean(y_gen - y_real)
            d_obj += self.grad_penalty * self.lipschitz_penalty(x_batch, x_gen)
        ...
        for t in range(self.total_epoch):
            ...
            if self.visualize:
                ...
                for i in range(vis_gen.shape[0]):
                    ...
                    plt.axis('off')
                plt.savefig("./{}_WGAN_Epoch_{:03d}.png".format(self.dataset, t + 1))
                plt.clf()
                plt.close(fig)
